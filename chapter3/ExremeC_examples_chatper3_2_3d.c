...
cartesian_pos_3d_t convert_to_3d_cartesian_pos(const polar_pos_3d_t *polar_pos) {
	...
	result.x = polar_pos->length * cos_deg(polar_pos->theta) * cos_deg(polar_pos->phi);
	result.y = polar_pos->length * sin_deg(polar_pos->theta) * cos_deg(polar_pos->phi);
result.z = polar_pos->length * cos_deg(polar_pos->theta) * cos_deg(polar_pos->phi);
...
polar_pos_3d_t convert_to_3d_polar_pos(const cartesian_pos_3d_t *cartesian_pos) {
	...
	result.theta = to_degree(atan(cartesian_pos->z / result.length));
