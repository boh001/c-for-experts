...
void shared_state_init(shared_state_t* shared_state) {
	...
	phtread_mutex_init(&shared_state->mtx, NULL);
	phtread_cond_init(&shared_state->cv, NULL);
}

void shared_state_destroy(shared_state_t* shared_state) {
	phtread_mutex_destroy(&shared_state->mtx);
	phtread_cond_destroy(&shared_state->cv);
}
...
int main() {
	...
	result1 = pthread_join(&thread1);
	result2 = pthread_join(&thread2);
